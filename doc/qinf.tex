...
\title{User's Manual for \qinf Quantum Information and Entanglement Package
 v. 0.07 For The \pname{Maxima} Computer
  Algebra System}
...
\end{comment}
   \item
    ...
    similar to other graphical frontends to \maximalink. Another popular graphical
    frontend is \hlink{\ipname{wxmaxima}}{http://wxmaxima.sourceforge.net}.
   \item
...
transpose function \maxman{\ifs{ctranspose}}{\maximactranspose}.
...
\subsection{\ifs{toproj} --- Density matrix representation of a pure state (projection operator)}
...
\subsection{\ifs{otimes} and \ifs{tensor\_product} \index{tensor product}
...
For \ifunc{ptrace}{\rho,i_1,\ldots} the density matrix $\rho$ is assumed to be in
...
\func{spinor\_rotation\_op}{phi,theta,gamma} retuns the matrix that represents
rotation through an angle \farg{gamma} about the axis specified by 
\farg{phi} and \farg{theta}. The function \func{spinor\_rotation\_op\_trig}{phi,theta,gamma}
returns the same matrix expressed only with cosines and sines. Explicitly the matrices are
\bbm
\begin{verbatim}
...
be relativley difficult to check by hand
...
because we will examine the reduced states, which are mixed. We compute
\begin{equation}
